...
-- 3) Customers in SP state (São Paulo) with a specific ZIP prefix range
...
-- 4) Orders with missing delivery date (potential delays/cancellations)
...
-- 5) Items that look unusually expensive (price outliers)
...
-- 6) Payments using credit card & installments >= 6
...
-- 7) Reviews with low scores and non-null comments
...
-- 8) Products with potential data quality issues (missing category)
...
-- 9) Seller concentration by state (quick scan)
SELECT seller_id, seller_city, seller_state
FROM sellers
WHERE seller_state IN ('SP','RJ','MG','RS')
LIMIT 20;

-- 10) Geolocation sample for ZIPs in SP
